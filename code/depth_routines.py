# A script for the Tutorial on Anomaly Detection
...
def depthPrj(X, data, ndirs = 100):
    """
    Routine approximates projection depth of X w.r.t. data
    Args:
    X - observations to calculate the depth for
    data - the sample defining the depth fucntion
    ndirs - number of directions
    Returns:
    An array of projection depths
    """
    # Generate directions
    d = len(data[0])
    dirsTmp = np.random.multivariate_normal(np.zeros(d), np.diag(np.ones(d)), ndirs)
    ...
    for j in range(ndirs):
        prjData = data @ dirs[j]
        prjMAD = np.median(np.abs(prjData - np.median(prjData)))
        for i in range(len(X)):
            prjCurX = X[i] @ dirs[j]
            curDepth = 1 / (1 + np.abs(prjCurX - np.median(prjData)) / prjMAD)
            depths[i] = min([depths[i], curDepth])
    return depths
